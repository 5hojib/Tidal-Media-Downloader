...
class TidalAPI:
    ...
    def __getResolutionList__(self, url):
        ...
        for item in array:
            ...
            stream.m3u8Url = (
                "http" + aigpy.string.getSubOnlyStart(item, "http").strip()
            )
            stream.resolution = aigpy.string.getSub(
                item, "RESOLUTION=", "http"
            ).strip()
    ...
    def verifyAccessToken(self, accessToken) -> bool:
        ...
        return not ("status" in result and result["status"] != 200)
    ...
    def loginByAccessToken(self, accessToken, userid=None) -> None:
        header = {"authorization": f"Bearer {accessToken}"}
        result = requests.get(
            "https://api.tidal.com/v1/sessions", headers=header
        ).json()
    ...
    def getPlaylist(self, id) -> Playlist:
        return aigpy.model.dictToModel(
            self.__get__("playlists/" + str(id)), Playlist()
        )
    ...
    def getLyrics(self, id) -> Lyrics:
        data = self.__get__(
            f"tracks/{id!s}/lyrics", urlpre="https://listen.tidal.com/v1/"
        )
    ...
    def getArtistAlbums(self, id, includeEP=False):
        data = self.__getItems__(f"artists/{id!s}/albums")
        albums = [aigpy.model.dictToModel(item, Album()) for item in data]
        if not includeEP:
            return albums

        data = self.__getItems__(
            f"artists/{id!s}/albums", {"filter": "EPSANDSINGLES"}
        )
        albums += [aigpy.model.dictToModel(item, Album()) for item in data]
        return albums
                ...
                for rep in adaptation_set.findall("Representation"):
                    ...
                    if seg_timeline is not None:
                        ...
                        for s in seg_timeline.findall("S"):
                            ...
                            # Segment reference
                            for _i in range(int(s.get("r") or 0) + 1):
    ...
    def getStreamUrl(self, id, quality: AudioQuality):
        ...
        data = self.__get__(f"tracks/{id!s}/playbackinfopostpaywall", paras)
        ...
        if "vnd.tidal.bt" in resp.manifestMimeType:
            ...
            ret.encryptionKey = manifest.get("keyId", "")
            ret.url = manifest["urls"][0]
            ret.urls = [ret.url]
            return ret
        if "dash+xml" in resp.manifestMimeType:
    ...
    def getVideoStreamUrl(self, id, quality: VideoQuality):
        ...
        data = self.__get__(f"videos/{id!s}/playbackinfopostpaywall", paras)
        resp = aigpy.model.dictToModel(data, StreamRespond())
    ...
    def getTrackContributors(self, id):
        return self.__get__(f"tracks/{id!s}/contributors")
    ...
    def parseUrl(self, url):
        ...
        url = url.lower()
        for _index, item in enumerate(Type):
