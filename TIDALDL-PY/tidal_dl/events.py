...
def start_album(obj: Album) -> None:
    Printf.album(obj)
...
def start_track(obj: Track) -> None:
...
def start_video(obj: Video) -> None:
    downloadVideo(obj, obj.album)


def start_artist(obj: Artist) -> None:
...
def start_playlist(obj: Playlist) -> None:
    Printf.playlist(obj)
...
def start_type(etype: Type, obj) -> None:
    if etype == Type.Album:
        start_album(obj)
...
def changeQualitySettings() -> None:
    Printf.settings()
...
def changeApiKey() -> bool:
    item = apiKey.getItem(SETTINGS.apiKeyIndex)
    ver = apiKey.getVersion()

    Printf.info(
        f'Current APIKeys: {SETTINGS.apiKeyIndex!s} {item["platform"]}-{item["formats"]}'
    )
    Printf.info(f"Current Version: {ver!s}")
...
def loginByWeb() -> bool | None:
    try:
        ...
        Printf.err(f"Login failed.{e!s}")
        return False


def loginByConfig() -> bool | None:
    try:
