...
def getAlbumPath(album) -> str:
    artistName = __fixPath__(TIDAL_API.getArtistsName(album.artists))
    albumArtistName = (
        __fixPath__(album.artist.name) if album.artist is not None else ""
    )

    # album folder pre: [ME]
    flag = TIDAL_API.getFlag(album, Type.Album, True, "")
    if SETTINGS.audioQuality not in (AudioQuality.Master, AudioQuality.Max):
        flag = flag.replace("M", "")
    if flag != "":
        flag = "[" + flag + "] "
...
def getPlaylistPath(playlist) -> str:
...
def getTrackPath(track, stream, album=None, playlist=None) -> str:
    base = "./"
    number = str(track.trackNumber).rjust(2, "0")
    if album is not None:
        base = getAlbumPath(album)
        if album.numberOfVolumes > 1:
            base += f"/CD{track.volumeNumber!s}"
...
def getVideoPath(video, album=None, playlist=None) -> str:
