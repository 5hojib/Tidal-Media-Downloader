...
if not enableGui():
    ...
    class MainView(QtWidgets.QWidget):
        ...
        def addItem(self, rowIdx: int, colIdx: int, text) -> None:
        ...
        def search(self) -> None:
            ...
            if self.s_text.startswith("http"):
                ...
                if tmpType == Type.Null:
                    ...
                    return
                if tmpType not in self.m_supportType:
        ...
        def setSearchResults(self, s_array, s_type) -> None:
        ...
        def __downloadFunc__(self, items):
            ...
            def __thread_download__(model: MainView, items) -> None:
                itemTitle = ""
                type = model.s_type
                try:
                    for item in items:
        ...
        def downloadEnd(self, title, result, msg) -> None:
            self.c_btnDownload.setEnabled(True)
            self.c_btnDownload.setText("Download")

            if result:
                self.__info__("Download finished.")
            else:
        ...
        def changeTQuality(self, index) -> None:
            SETTINGS.audioQuality = self.c_combTQuality.itemData(index)
            SETTINGS.save()

        def changeVQuality(self, index) -> None:
